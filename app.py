...
if check_password():
    ...
    # 2. DISEÑO Y LOGO
    st.set_page_config(page_title="Asistente de Relevamiento - Agentes 2026", page_icon="📋")
    ...
    st.markdown("<h2 style='text-align: center; color: #1E88E5;'>Asistente de Datos - Agentes 2026 🤖</h2>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if not st.session_state["admin_desbloqueado"]:
            clave_admin = st.text_input("Clave para ver DNI / Nombres / Contacto:", type="password")
            ...
            st.success("🔓 Modo Administrador Activo (Datos sensibles visibles)")
    ...
    def cargar_datos():
        url = "https://docs.google.com/spreadsheets/d/1FlTvtClaQYOXwJU473PXr0DbnN05Wdzpt-HgiF1wEoQ/export?format=csv&gid=0"
        df = pd.read_csv(url)
        
        # Eliminar filas completamente vacías
        df = df.dropna(how='all')

        # Formatear la primera columna de ID a 4 dígitos si aplica
        col_id = df.columns[0]
        ...
        # Estandarización de nombres de coordenadas para st.map (requiere 'lat' y 'lon')
        cols_lower = {c: c.lower().strip() for c in df.columns}
        df = df.rename(columns=cols_lower)
        
        if 'latitud' in df.columns and 'lat' not in df.columns:
            df = df.rename(columns={'latitud': 'lat'})
        if 'longitud' in df.columns and 'lon' not in df.columns:
    ...
    columnas_sensibles = ['dni', 'apellido', 'apellidos', 'nombre', 'nombres', 'telefono', 'celular', 'email']
    ...
    with st.expander("Ver primeros datos del relevamiento"):
    ...
    pregunta = st.chat_input("Ej: ¿Cuántas viviendas tienen riesgo de basural?")

    if pregunta:
        palabras_sensibles = ["dni", "documento", "apellido", "nombre", "telefono", "celular"]
        ...
        if requiere_sensibles and not st.session_state["admin_desbloqueado"]:
            ...
            with st.chat_message("assistant", avatar=AVATAR_BOT):
                msg_bloqueo = "🔒 **Acceso Denegado:** Esta consulta solicita datos protegidos (DNI, nombres, apellidos o contacto). Para verlos debes ingresar la clave especial en el menú lateral."
            ...
            with st.chat_message("assistant", avatar=AVATAR_BOT):
                with st.spinner("Analizando relevamiento y procesando respuesta... ⏳"):
...
2. Guarda la conclusión o respuesta numérica en texto amigable y legible dentro de la variable `respuesta_final`.
3. BÚSQUEDAS Y FILTROS:
   - Para columnas con casillas de verificación (ej. booleanos True/False, 'TRUE', 'FALSE', o símbolos), considera que pueden venir como booleanos o cadenas de texto. Usa comparaciones seguras.
   - Para columnas de texto (ej. 'limpieza', 'tipo_vivienda', 'material_techo', etc.), usa siempre `.astype(str).str.contains(r'patron', case=False, na=False)`.
